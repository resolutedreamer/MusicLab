...
//This gpio for on or off
int onOroff = 7;
//Binary representation of track number
int msb = 6;
int lsb = 5;
//Reply to the master arduino
int reply = 4;
//messages to master device
int pause_master = 3;
int stop_servo = 2;
...
void setup() {
  ...
  pinMode(pause_master, OUTPUT);
  pinMode(stop_servo, OUTPUT);
  pinMode(pir, INPUT);
}
...
void loop() {
  ...
  while (i < len1 && digitalRead(onOroff) == HIGH && digitalRead(msb) == LOW && digitalRead(lsb) == LOW) {
    while (digitalRead(pauc) == HIGH) {
      digitalWrite(pause_master, HIGH);
    }
    delay(5);
    digitalWrite(pause_master, LOW);
    tone(soundpin, notes1[i], beats1[i]);
    if (digitalRead(pauc_m) != HIGH || digitalRead(pir) == LOW) {
      digitalWrite(stop_servo, LOW);
      if (i % 2 == 0) {
  ...
  i = 0;
  while (i < len2 && digitalRead(onOroff) == HIGH && digitalRead(msb) == LOW && digitalRead(lsb) == HIGH) {
    while (digitalRead(pauc) == HIGH || digitalRead(pir) == LOW) {
      digitalWrite(pause_master, HIGH);
    }
    delay(5);
    digitalWrite(pause_master, LOW);
    tone(soundpin, notes2[i], beats2[i]);
    if (digitalRead(pauc_m) != HIGH) {
      digitalWrite(stop_servo, LOW);
      if (i % 2 == 0) {
        digitalWrite(vibpin2, HIGH);
      } else {
  ...
  i = 0;
  while (i < len3 && digitalRead(onOroff) == HIGH && digitalRead(msb) == HIGH && digitalRead(lsb) == LOW) {
    while (digitalRead(pauc) == HIGH || digitalRead(pir) == LOW) {
      digitalWrite(pause_master, HIGH);
    }
    delay(5);
  ...
  i = 0;
  while (i < len4 && digitalRead(onOroff) == HIGH && digitalRead(msb) == HIGH && digitalRead(lsb) == HIGH) {
    while (digitalRead(pauc) == HIGH || digitalRead(pir) == LOW) {
      digitalWrite(pause_master, HIGH);
    }
    delay(5);
